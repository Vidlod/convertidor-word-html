...
class Validator(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'p' and self.in_li:
            self.errors.append("Nested <p> inside <li> tag found.")
            
        if tag == 'em':
            self.errors.append("Forbidden italic tag <em> found.")
    ...
    def handle_endtag(self, tag):
        ...
        if tag == 'li':
            self.in_li = False
            # Check if this <li> is a navigation tab item (should not end with period)
            is_nav = 'nav-item' in self.li_attrs.get('class', '') or 'nav-link' in self.li_attrs.get('class', '')
            if not is_nav:
                # Also check parent tags
                for ptag, pattrs in self.tag_stack:
                    pattrs_dict = dict(pattrs)
                    if 'nav' in pattrs_dict.get('class', '') or 'nav-tabs' in pattrs_dict.get('class', ''):
                        is_nav = True
                        break
            ...
            if not is_nav:
                ...
                if clean_text:
                    if not clean_text.endswith('.') and not clean_text.endswith('?') and not clean_text.endswith(':'):
                        self.errors.append(f"<li> element text does not end with a period/question/colon: {repr(clean_text[:40])}...")
...
def validate_file(path):
    ...
    # 5b. Check for unlinked key documents in paragraphs (Formato/Forma Rule)
    ...
    for match in doc_matches:
        idx = match.start()
        surrounding = content[max(0, idx - 500):idx + len(match.group()) + 500]
        # Ignore if part of a button, rae bibliography or if already linked
    ...
    # 5b. Check list markers outside of strong tags
    outside_markers = re.findall(r'\b[a-z0-9](\.|\))\s*<strong[^>]*>', content, re.IGNORECASE)
    if outside_markers:
...
if __name__ == "__main__":
    ...
    print("\n========================================\nREPORT SUMMARY:")
    if all_success:
        print("🎉 ALL FILES ARE 100% COMPLIANT AND READY FOR MOODLE!")
        sys.exit(0)
    else:
        print("⚠️ COMPLIANCE FAILURE: Please review and fix the errors listed above before uploading to Moodle.")
        sys.exit(1)
