...
with open("/Users/buc-cvudes-medios1/Documents/GEO/scratch/docx_rows.txt", "r", encoding="utf-8") as f:
    lines = f.read()
...
for idx, r in enumerate(parser.rows):
    if len(r) > 0:
        first = r[0].strip()
        # if first is a digit or "Semana" or "CIERRE"
        if first.isdigit() or first.startswith("Semana") or "CIERRE" in first:
            col1 = r[1][:150] if len(r)>1 else ""
            print(f"Row {idx:2d} (Col count {len(r)}): First col: '{first}'")
            print(f"       Second col snippet: '{col1}'")
