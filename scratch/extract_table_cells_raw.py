...
class RawTableParser(HTMLParser):
    def __init__(self):
        super().__init__()
        self.in_table = False
        self.in_cell = False
...
print("Number of rows found:", len(parser.rows))

with open("/Users/buc-cvudes-medios1/Documents/GEO/scratch/weeks_raw_cells.txt", "w", encoding="utf-8") as f:
    for idx, r in enumerate(parser.rows):
        if len(r) > 0:
            first_col_text = " ".join(r[0].split()).strip()
            # remove tags from first col to check if digit or week
            import re
            ...
            if first_col_clean.isdigit() or first_col_clean.startswith("Semana") or "CIERRE" in first_col_clean:
                f.write(f"============================================================\n")
                f.write(f"ROW {idx} - WEEK: {first_col_clean}\n")
                f.write(f"============================================================\n")
                for col_idx, col in enumerate(r):
                    f.write(f"--- Col {col_idx} ---\n{col}\n\n")

print("Dumped raw cells to scratch/weeks_raw_cells.txt")
